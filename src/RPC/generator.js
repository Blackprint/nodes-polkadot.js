// Work in progress
let SubstrateMetadata = false; // set this to true to enable this generator
...
function Substrate_BlackprintNodeGenerator(list){
	...
	that: for (var i = 0; i < list.length; i++) {
		let temp = list[i];
		...
		for (var a = 0; a < funcs.length; a++) {
			...
			// /(Get|Set|Has|New)[A-Z](.*?By[A-Z])?/

			Blackprint.registerNode("Polkadot.js/RPC/"+temp.name+'/'+func.name,
			class GeneratedNode extends Blackprint.Node {
				constructor(instance){
					super(instance);

					let iface = this.setInterface();
					iface.title = func.name;

					if(func.args !== void 0)
						this.input = Object.assign({}, func.args);
